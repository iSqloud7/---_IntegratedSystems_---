...
public static class SubmissionHelper
{
    public static void ZipProject(string rootPath, string outputZip)
    {
        ...
        foreach (var file in Directory.EnumerateFiles(rootPath, "*.*", SearchOption.AllDirectories).Where(x => !x.EndsWith(".vsidx") && !x.EndsWith(".testlog") && !x.EndsWith(".manifest") && !x.Contains(@"\.vs\") && !x.Contains(@"/.vs/")))
        {
